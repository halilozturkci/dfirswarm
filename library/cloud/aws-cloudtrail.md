...
dated rows built from the ledger, each row naming the identity,
`work/indicators.md` holds one table of every indicator (type, value,
first seen, identity, source record, confidence; one row saying so if
none was found), the report ends question 8 with the remediation list,
...
- `head -3 work/timeline.md | grep -qi 'identity'`
